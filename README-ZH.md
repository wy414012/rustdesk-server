...
`relay-server-ip` 参数是运行这些容器的服务器的 IP 地址（或 DNS 名称）。如果你不想使用 **41117** 作为 `hbbr` 的服务端口,可使用可选参数 `port` 进行指定。
...
编辑第16行来指定你的中继服务器 （默认端口监听在 41117 的那一个）。 如果需要的话，您也可以编辑 volume 信息  (第 18 和 33 行)。
...
| PORT                  | hbbs/hbbr     | 监听端口（hbbs为41116，hbbr为41117）                      |
